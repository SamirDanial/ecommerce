...
const OrderTrackingModal: React.FC<OrderTrackingModalProps> = ({
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto p-4 sm:p-6">
          <DialogHeader className="mb-4 sm:mb-6 text-center">
            ...
            <p className="text-gray-600 mt-2 text-sm sm:text-base">
              Gathering the latest tracking information...
            </p>
          </DialogHeader>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto p-4 sm:p-6">
          <DialogHeader className="mb-4 sm:mb-6 text-center">
            ...
            <p className="text-gray-600 mt-2 text-sm sm:text-base">
              We're working on getting your tracking details ready
            </p>
          </DialogHeader>
        </DialogContent>
      </Dialog>
  ...
  const currentStatusInfo = trackingService.getStatusInfo(
    trackingData.currentStatus
  );
  const progressPercentage = trackingService.calculateProgress(
    trackingData.currentStatus
  );
          ...
          <div className="border border-gray-200 rounded-lg p-3 sm:p-6">
            ...
            <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 sm:gap-6 mb-4 sm:mb-6">
              <div className="flex items-center gap-3 sm:gap-4">
                <div
                  className={`p-3 sm:p-4 rounded-xl ${currentStatusInfo.bgColor} shadow-sm`}
                >
                  <div
                    className={`text-2xl sm:text-3xl ${currentStatusInfo.color}`}
                  >
          ...
          <div className="border border-gray-200 rounded-lg p-3 sm:p-6">
            ...
            <div className="space-y-3 sm:space-y-4">
              {trackingData.statusHistory?.map((event: any, index: number) => (
                <div
                  key={index}
                  className="flex items-start gap-3 sm:gap-4 p-3 sm:p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors"
                >
