...
function createSVGPlaceholder(width: number, height: number, text: string, color: string = '#6B7280'): string {
  return `<svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
    <rect width="${width}" height="${height}" fill="#F3F4F6"/>
    <rect x="2" y="2" width="${width-4}" height="${height-4}" fill="#E5E7EB" stroke="#D1D5DB" stroke-width="1"/>
    <text x="${width/2}" y="${height/2 + 5}" font-family="Arial, sans-serif" font-size="12" text-anchor="middle" fill="${color}">${text}</text>
  </svg>`;
}
...
async function main() {
  ...
  try {
    ...
    for (const productInfo of productData) {
      ...
      console.log(`✅ Created ${images.length} SVG images for ${product.name}`);
    }

    console.log('\n🎉 Product seeding with images completed successfully!');
    console.log(`📊 Created ${productData.length} products with variants and SVG images`);
